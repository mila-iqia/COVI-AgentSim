...
import covid19sim.distributed_inference.clustering.simple as clu
...
class SimpleClusteringTests(unittest.TestCase):
    ...
    def test_same_day_visit_clusters(self):
        ...
        for _ in range(n_trials):
            ...
            visits = [
                Visit(visitor_real_uid=1, visited_real_uid=0, exposition=False, timestamp=Toff(2)),
                Visit(visitor_real_uid=2, visited_real_uid=0, exposition=False, timestamp=Toff(2)),
            ]
            # we will cheat and hard-code some initial 4-bit uids to make sure there is no overlap
            humans = [
                FakeHuman(
                    real_uid=idx,
                    exposition_timestamp=Toff(9999),
    ...
    def test_same_day_visit_clusters_overlap(self):
        ...
        visits = [
            ...
            Visit(visitor_real_uid=2, visited_real_uid=0, exposition=False, timestamp=Toff(0)),
            Visit(visitor_real_uid=3, visited_real_uid=0, exposition=False, timestamp=Toff(0)),
            Visit(visitor_real_uid=4, visited_real_uid=0, exposition=False, timestamp=Toff(0)),
            Visit(visitor_real_uid=5, visited_real_uid=0, exposition=False, timestamp=Toff(0)),
        ]
        ...
        humans = [
            FakeHuman(
                ...
                exposition_timestamp=Toff(9999),
                visits_to_adopt=visits,
                force_init_uid=max(idx - 1, 0) % 3,
                force_init_risk=mu.RiskLevelType(7),
            ) for idx in range(6)
        ]
    ...
    def test_cluster_risk_update(self):
        ...
        humans = [
            FakeHuman(real_uid=0, exposition_timestamp=Toff(9999), visits_to_adopt=visits,
                      force_init_risk=mu.RiskLevelType(0)),
        ...
        self.assertEqual(cluster_manager.clusters[0].risk_level, mu.RiskLevelType(7))
        # we will add a manual update for one of the visits
        cluster_manager.add_messages([
            mu.create_update_message(h0_messages[0], mu.RiskLevelType(9), Toff(1))
        ])
        self.assertEqual(len(cluster_manager.clusters), 1)
        # add a new encounter: it should not match the existing cluster due to diff risk
        cluster_manager.add_messages([
            mu.EncounterMessage(humans[1].rolling_uids[0], risk_level=mu.RiskLevelType(1), encounter_time=Toff(0))
        ])
        self.assertEqual(len(cluster_manager.clusters), 2)
        self.assertEqual(cluster_manager.clusters[1].risk_level, mu.RiskLevelType(1))
        # add a new encounter: it should match the existing cluster due to same risk
        new_encounter = \
            mu.EncounterMessage(humans[1].rolling_uids[0], risk_level=mu.RiskLevelType(7), encounter_time=Toff(0))
        ...
        cluster_manager.add_messages([
            mu.create_update_message(new_encounter, mu.RiskLevelType(13), Toff(1))
        ])
        self.assertEqual(len(cluster_manager.clusters), 2)
        self.assertEqual(len(cluster_manager.clusters[0].messages), 2)
    ...
    def test_cleanup_outdated_cluster(self):
        ...
        visits = [
            Visit(visitor_real_uid=1, visited_real_uid=0, exposition=False, timestamp=Toff(2)),
            Visit(visitor_real_uid=1, visited_real_uid=0, exposition=False, timestamp=Toff(5)),
        ...
        h0_messages = [msg for msgs in h0_messages.values() for msg in msgs]
        cluster_manager = clu.SimplisticClusterManager(max_history_offset=datetime.timedelta(days=5))
        cluster_manager.add_messages(h0_messages)
        self.assertEqual(len(cluster_manager.clusters), 2)
        self.assertEqual(cluster_manager.clusters[0].first_update_time, Toff(5))
        self.assertEqual(cluster_manager.clusters[1].first_update_time, Toff(8))
