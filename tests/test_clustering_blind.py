...
class BlindClusteringTests(unittest.TestCase):
    ...
    def test_same_day_visit_clusters(self):
        ...
        for _ in range(n_trials):
            ...
            humans = [FakeHuman(real_uid=idx, exposition_timestamp=Toff(9999), visits_to_adopt=visits)
                      for idx in range(3)]
            ...
            humans = [FakeHuman(real_uid=idx, exposition_timestamp=Toff(9999),
    ...
    def test_cluster_risk_update(self):
        ...
        self.assertEqual(len(cluster_manager.clusters), 1)
        self.assertEqual(cluster_manager.clusters[0].risk_level, mu.RiskLevelType(9))
        # add a new encounter: it should not match the existing cluster due to diff risk
        cluster_manager.add_messages([
            mu.EncounterMessage(mu.create_new_uid(), risk_level=mu.RiskLevelType(1), encounter_time=Toff(0))
        ])
        self.assertEqual(len(cluster_manager.clusters), 2)
        ...
        new_encounter = \
            mu.EncounterMessage(mu.create_new_uid(), risk_level=mu.RiskLevelType(9), encounter_time=Toff(0))
        ...
        self.assertEqual(cluster_manager.clusters[0].risk_level, mu.RiskLevelType(9))
        self.assertEqual(cluster_manager.clusters[1].risk_level, mu.RiskLevelType(13))
        self.assertEqual(cluster_manager.clusters[2].risk_level, mu.RiskLevelType(1))
    ...
    def test_cleanup_outdated_cluster(self):
        ...
        messages = generate_received_messages(humans)
        h0_messages = [msg for msgs in messages[0]["received_messages"].values() for msg in msgs]
        cluster_manager = clu.BlindClusterManager(max_history_offset=datetime.timedelta(days=5))
        cluster_manager.add_messages(h0_messages)
        self.assertEqual(len(cluster_manager.clusters), 2)
