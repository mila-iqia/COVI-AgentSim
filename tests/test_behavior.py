...
from covid19sim.epidemiology.p_infection import get_human_human_p_transmission
...
class BehaviorTest(unittest.TestCase):
    def setUp(self):
        ...
        self.human1.age = 20
        self.human1.normalized_susceptibility = self.conf['NORMALIZED_SUSCEPTIBILITY_BY_AGE'][2][-1]
        self.human1.mean_daily_interaction_age_group = self.conf['MEAN_DAILY_INTERACTION_FOR_AGE_GROUP'][2][-1]
        ...
        self.human2.age = 20
        self.human2.normalized_susceptibility = self.conf['NORMALIZED_SUSCEPTIBILITY_BY_AGE'][2][-1]
        self.human2.mean_daily_interaction_age_group = self.conf['MEAN_DAILY_INTERACTION_FOR_AGE_GROUP'][2][-1]
        ...
        self.default_p_infection = get_human_human_p_transmission(
    ...
    def test_wash_hands_p_infection(self):
        ...
        infector = self.human1
        infectee = self.human2
        p_infection = get_human_human_p_transmission(infector, infector.infectiousness, infectee, self.location_social_contact_factor, self.conf["CONTAGION_KNOB"], self.conf['MASK_EFFICACY_FACTOR'], self.conf['HYGIENE_EFFICACY_FACTOR'], self.human1, self.human2)
