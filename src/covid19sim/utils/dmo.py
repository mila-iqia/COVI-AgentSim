...
def assign_households_to_humans(humans, city, conf, logfile=None):
    ...
    OTHER_KID_BINS = _get_valid_bins(AGE_BIN_WIDTH_5, min_age=MAX_AGE_CHILDREN, max_age=MAX_AGE_WITH_PARENT)
    ...
    while True:
        ...
        if n_kids_needed != 0 and len(valid_kid_bins) == 0:
            SEARCH_BINS = OTHER_KID_BINS # expand the search to middle generation
            continue
        ...
        # Note: We sample from YOUNGER_BINS while sampling for other kids in _sample_other_residents
...
def _sample_other_residents(housetype, unassigned_humans, rng, conf, with_kid=None):
    ...
    if with_kid is not None:
        ...
        if housetype.living_arrangement == "solo":
            ...
            valid_couple_bins = valid_age_bins
