...
def assign_households_to_humans(humans, city, conf, logfile=None):
    ...
    # allocate households
    # presample house sizes equal to the number of houses as per census
    # allocate housesizes > 2 first because it's difficult to find the best fit for them at the end
    n_houses_approx = int(n_people / AVG_HOUSEHOLD_SIZE)
    housesizes = city.rng.choice(range(1,6), p=P_HOUSEHOLD_SIZE, size=n_houses_approx)
    # presampled_housesizes = housesizes[housesizes == 1].tolist()
    # presampled_housesizes += housesizes[(housesizes == 2) + (housesizes == 5)].tolist()
    # presampled_housesizes += housesizes[(housesizes == 3) + (housesizes == 4)].tolist()

    # presampled_housesizes = np.sort(housesizes).tolist()
    # presampled_housesizes = np.sort(housesizes[(housesizes == 2) + (housesizes == 5)]).tolist()
    # presampled_housesizes += np.sort(housesizes[(housesizes == 3) + (housesizes == 4)]).tolist()
    # presampled_housesizes += housesizes[housesizes == 1].tolist()

    # presampled_housesizes = np.sort(housesizes[housesizes != 1])[::-1].tolist()
    # presampled_housesizes += housesizes[housesizes == 1].tolist()

    # presampled_housesizes = np.sort(housesizes)[::-1].tolist()

    presampled_housesizes = housesizes[housesizes <= 2].tolist()
    presampled_housesizes += housesizes[housesizes > 2].tolist()

    # P_HOUSEHOLD_ABOVE_SIZE_1 = [x / sum(P_HOUSEHOLD_SIZE[1:]) for x in P_HOUSEHOLD_SIZE[1:]]
    n_failed_attempts, deviation_type, deviation_size = 0, 0, 0
    while len(allocated_humans) < city.n_people and n_failed_attempts < MAX_FAILED_ATTEMPTS_ALLOWED:
        if len(presampled_housesizes) > 0:
            housesize = presampled_housesizes.pop()
        else:
            # print("starting extra sampling ... ")
            # housesize = city.rng.choice(range(2,6), p=P_HOUSEHOLD_ABOVE_SIZE_1, size=1).item()
        ...
        if humans_with_same_house:
            ...
            # print(f"failed housesize:{housesize}. Failed attempt - {n_failed_attempts}. Deviation: type {deviation_type} size {deviation_size}. Total allocated:{len(allocated_humans)}")
            deviation_size += 1
            n_failed_attempts += 1
            if n_failed_attempts % 100 == 0:
                log(f"Failed attempt - {n_failed_attempts}. Deviation: type {deviation_type} size {deviation_size}. Total allocated:{len(allocated_humans)}", logfile)
    ...
    if n_failed_attempts >= MAX_FAILED_ATTEMPTS_ALLOWED:
        log(f"Deviation: type {deviation_type} size {deviation_size}. Exceeded the maximum number of failed attempts allowed to allocate houses... trying random allocation!", logfile)
    ...
    log(f"Housing allocated with deviation: type {deviation_type} size {deviation_size} ", logfile)
    return allocated_humans
...
def find_two_humans_for_house(conf, city, unassigned_humans):
    ...
    while n_iters < MAX_TRIES:
        ...
        if type == "couple":
            ...
            two_humans = _sample_couple_with_n_kids(valid_couple_bins, unassigned_humans, city.rng, n=0)

        elif type == "single_parent":
            valid_older_bins = _get_valid_bins(valid_age_bins, MIN_AGE_SINGLE_PARENT, MAX_AGE_SINGLE_PARENT)
            ...
            two_humans = _sample_random_humans(valid_other_bins, unassigned_humans, city.rng, size=2)
...
def find_three_humans_for_house(conf, city, unassigned_humans):
    ...
    while n_iters < MAX_TRIES:
        ...
        if type == "couple_with_kid":
            valid_couple_bins = _get_valid_bins(valid_age_bins, min_age=MIN_AGE_COUPLE, max_age=MAX_AGE_COUPLE_WITH_CHILDREN)
            valid_younger_bins = _get_valid_bins(valid_age_bins, max_age=MAX_AGE_CHILDREN)
            ...
            # (no-source) all other type of housing is resided by adults only
            valid_other_bins = _get_valid_bins(valid_age_bins, min_age=MAX_AGE_CHILDREN)
            three_humans = _sample_random_humans(valid_other_bins, unassigned_humans, city.rng, size=3)
...
def find_four_humans_for_house(conf, city, unassigned_humans):
    ...
    while n_iters < MAX_TRIES:
        ...
        if type == "couple_with_two_kids":
            ...
            # (no-source) all other type of housing is resided by adults only
            valid_other_bins = _get_valid_bins(valid_age_bins, min_age=MAX_AGE_CHILDREN)
            four_humans = _sample_random_humans(valid_other_bins, unassigned_humans, city.rng, size=4)
...
def find_five_humans_for_house(conf, city, unassigned_humans):
    ...
    while n_iters < MAX_TRIES:
        ...
        if type == "couple_with_three_kids":
            ...
            valid_other_bins = _get_valid_bins(valid_age_bins, min_age=MAX_AGE_CHILDREN)
            five_humans = _sample_random_humans(valid_other_bins, unassigned_humans, city.rng, size=5)
...
def _sample_couple_with_n_kids(couple_bins, unassigned_humans, rng, n=0, younger_bins=[]):
    ...
    sequential_couple_bins = [
                            ...
                            if (len(unassigned_humans[x]) >= 1
                                ...
                                and x in couple_bins
                                and y in couple_bins)
                            ]

    valid_couple_bins = min_couple_single_bins + sequential_couple_bins

    # filter candidates for kids
    all_kids = [(y,x) for x in younger_bins for y in unassigned_humans[x]]
