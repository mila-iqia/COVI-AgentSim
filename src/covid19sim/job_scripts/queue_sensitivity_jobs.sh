...
APP_UPTAKE=(0.8415 0.7170 0.5618 0.4215 0.2850 1.005 1.4790 100) # 70% - 90% at end to avoid breaking other types
...
if [ "$TYPE" == "cost-benefit-analysis" ]; then
  Lx=${ALL_LEVELS_DROPOUT[$ALL_LEVELS_DROPOUT_DEFAULT_INDEX]}
  Sx=${P_DROPOUT_SYMPTOM[$P_DROPOUT_SYMPTOM_DEFAULT_INDEX]}
  Tx=${PROPORTION_LAB_TEST_PER_DAY[$PROPORTION_LAB_TEST_PER_DAY_DEFAULT_INDEX]}
  Ax=${BASELINE_P_ASYMPTOMATIC[$BASELINE_P_ASYMPTOMATIC_DEFAULT_INDEX]}
  FOLDER_NAME=${NAME}/cost-benefit
  ...
  if [ "$ACTION" == "launch-jobs" ]; then
    ...
    for intervention in transformer
    # for intervention in bdt1 heuristicv4 transformer
    do
      ./launch_mobility_experiment.sh Main ${APP_UPTAKE[4]} $Ax $Lx $Sx $Tx \
                $intervention $FOLDER_NAME $TYPE $N_PEOPLE $INIT
    done
    ...
    # for intervention in bdt1 heuristicv4 transformer
    do
      ./launch_mobility_experiment.sh Main ${APP_UPTAKE[2]} $Ax $Lx $Sx $Tx \
                $intervention $FOLDER_NAME $TYPE $N_PEOPLE $INIT
    done
    ...
    # # for intervention in bdt1 heuristicv4 transformer
    # do
    #   ./launch_mobility_experiment.sh Main ${APP_UPTAKE[1]} $Ax $Lx $Sx $Tx \
    #             $intervention $FOLDER_NAME $TYPE $N_PEOPLE $INIT
    # done
    ...
    # # for intervention in bdt1 heuristicv4 transformer
    # do
    #   ./launch_mobility_experiment.sh Main ${APP_UPTAKE[5]} $Ax $Lx $Sx $Tx \
    #             $intervention $FOLDER_NAME $TYPE $N_PEOPLE $INIT
    # done
    ...
    # for intervention in bdt1 heuristicv4 transformer
    # do
    #   ./launch_mobility_experiment.sh Main ${APP_UPTAKE[6]} $Ax $Lx $Sx $Tx \
    #             $intervention $FOLDER_NAME $TYPE $N_PEOPLE $INIT
    # done
    ...
    # for intervention in bdt1 heuristicv4 transformer
    # do
    #   ./launch_mobility_experiment.sh Main ${APP_UPTAKE[7]} $Ax $Lx $Sx $Tx \
    #             $intervention $FOLDER_NAME $TYPE $N_PEOPLE $INIT
    # done
