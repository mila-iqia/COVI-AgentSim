...
SENSITIVITY_PARAMETERS = ['ASYMPTOMATIC_RATIO', 'ALL_LEVELS_DROPOUT', 'P_DROPOUT_SYMPTOM',  'PROPORTION_LAB_TEST_PER_DAY', 'BASELINE_P_ASYMPTOMATIC', 'GLOBAL_MOBILITY_SCALING_FACTOR', "ASYMPTOMATIC_INFECTION_RATIO"]
...
def plot_and_save_mobility_scatter(results, uptake_rate, xmetric, ymetric, path, USE_GP=False, plot_residuals=False, display_r_squared=False, annotate_advantages=True, plot_scatter=True, plot_heatmap=True, trend_fit=""):
    ...
    """
    assert xmetric in results.columns and ymetric in results.columns, f"Unknown metrics: {xmetric} or {ymetric}. Expected one of {results.columns}."
    TICKGAP=2
    ANNOTATION_FONTSIZE=15
