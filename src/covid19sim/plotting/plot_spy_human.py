...
def load_humans(data):
    ...
    for i in data['human_monitor'].keys():
        ...
        hm = sorted(data['human_monitor'][i], key=lambda x:int(x['name'].split(":")[-1]))
...
def summarize(x, human_risk_each_day, to_human_max_msg_per_day, infectee_location, infector_infectee_update_messages):
    location_html = {
        "household": "&#127968",
        "hospital":"&#127973",
    ...
    for idx in range(len(columns)):
        ...
        if idx+1 < len(columns):
            ...
            messages = sorted(infector_infectee_update_messages[infector_name][infectee_name].items(), key=lambda x:x[0])
            for timestamp, payload in messages:
                if timestamp.date() == today:
                    ...
                    if payload['contact']:
                        ...
                        content = f'C{count}'
                        max_risk_level = max(max_risk_level, payload['contact']['new_risk_level'])
                    
                    if payload['unknown']:
                        count = payload['unknown']['count']
                        content += f'U{count}'
                        max_risk_level = max(max_risk_level, payload['unknown']['new_risk_level'])
                    if content:
                        cell += f' \u2709 {content} \u2b95'
                    else:
                        raise
...
def spy_pro(ids, humans, human_risk_each_day, to_human_max_msg_per_day, infectee_location, human_has_app, infector_infectee_update_messages, human_is_asymptomatic):
    df = humans.loc[(slice(None), ("infection_timestamp", "risk", "test_result", "n_symptoms", "risk_level", "rec_level", "reported_test_result", 'n_reported_symptoms')), ids]

    # display until here
    notnull = pd.notnull(df[ids[-1]].unstack().infection_timestamp)
    last_index = notnull[::-1].idxmax() + datetime.timedelta(days=2)

    df = df.groupby(level=0, axis=0).apply(lambda x: summarize(x, human_risk_each_day, to_human_max_msg_per_day, infectee_location, infector_infectee_update_messages))
    ...
    df.columns = [f"{x} ({is_asymptomatic[idx]}, {has_app[idx]})" for idx,x in enumerate(df.columns)]
...
def plot(input_file, output_file):
    ...
    df = spy_pro(ids, humans, human_risk_each_day, to_human_max_msg_per_day, infectee_location, human_has_app, infector_infectee_update_messages, human_is_asymptomatic)
